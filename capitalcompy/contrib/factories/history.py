...
import capitalcompy.endpoints.prices as prices
from capitalcompy.contrib.generic import granularity_to_time, secs2time
...
def EpicCandlesFactory(epic, params=None):
    ...
    *to*. If *to* is not specified *to* will be equal to *now*.
    ...
    # if not specified use the default of 'M' as CAPITAL.COM does
    ...
    # CAPITAL.COM will respond with a V10Error if count > MAX_BATCH
    ...
    if not params or 'from' not in params:
        ...
        for _ in range(_count, int(((nbars//_count)+1))*_count+1, _count):
            ...
            yield prices.EpicCandles(epic=epic,
                                                 params=yparams)
            _epoch_from = to
